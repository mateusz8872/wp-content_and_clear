...
if ( ! class_exists( 'Hester_SocialSnap' ) ) :
	/**
	 * Social Snap compatibility class.
	 */
	class Hester_SocialSnap {
		...
		public function __construct() {
			...
			add_filter( 'hester_default_option_values', array( $this, 'default_customizer_values' ), 20 );

			// Remove Social Snap Lite from recommended plugins.
			add_filter( 'hester_recommended_plugins', array( $this, 'update_recommended_plugins' ) );

			// Include helper functions.
			require HESTER_THEME_PATH . '/inc/compatibility/socialsnap/socialsnap-functions.php'; // phpcs:ignore
		}
		...
		public function register_options( $options ) {
			...
			$options['setting']['hester_single_post_meta_elements']['control']['choices']['shares'] = esc_html__( 'Shares', 'hester' );

			$options['setting']['hester_blog_entry_meta_elements']['control']['choices']['shares'] = esc_html__( 'Shares', 'hester' );
		...
		public function default_customizer_values( $defaults ) {
			...
			$defaults['hester_single_post_meta_elements']['shares'] = false;
			$defaults['hester_blog_entry_meta_elements']['shares']  = false;
