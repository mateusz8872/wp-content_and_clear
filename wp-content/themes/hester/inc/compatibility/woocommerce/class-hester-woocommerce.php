...
if ( ! hester_is_woocommerce_activated() ) {
	add_action( 'activate_woocommerce/woocommerce.php', array( hester_dynamic_styles(), 'delete_dynamic_file' ) );
	return;
}

/**
 * Hester WooCommerce Compatibility.
 */
if ( ! class_exists( 'Hester_Woocommerce' ) ) :
	...
	class Hester_Woocommerce {
		 ...
		 *
		 * @since 1.0.0
		 * @return Hester_Woocommerce
		 */
		public static function instance() {

			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Hester_Woocommerce ) ) {
				self::$instance = new Hester_Woocommerce();
		...
		private function includes() {
			...
			require HESTER_THEME_PATH . '/inc/compatibility/woocommerce/woocommerce-functions.php'; // phpcs:ignore
			require HESTER_THEME_PATH . '/inc/compatibility/woocommerce/class-hester-customizer-woocommerce.php'; // phpcs:ignore
		}
		...
		private function actions() {
			...
			if ( ! is_admin() ) {
				...
				// Disable Hester page description.
				add_filter( 'hester_page_header_description', array( $this, 'shop_remove_page_description' ) );
				...
				// Extend Hester breadcrumb trail.
				add_filter( 'hester_breadcrumb_trail_items', array( $this, 'breadcrumbs' ), 20, 2 );
				add_filter( 'hester_post_type_archive_title', array( $this, 'breadcrumb_post_type_archive_title' ), 10, 2 );

				// Remove WooCommerce sidebar.
				remove_action( 'woocommerce_sidebar', 'woocommerce_get_sidebar' );
				add_action( 'hester_woocommerce_sidebar', 'woocommerce_get_sidebar' );
				...
				// Replace WooCommerce pagination with Hester pagination.
				remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );
				add_action( 'woocommerce_after_shop_loop', 'hester_pagination' );

				// Add back to shop button to Empty Cart.
				add_action( 'woocommerce_cart_is_empty', 'hester_wc_empty_cart_button' );
				...
				add_filter( 'woocommerce_sale_flash', 'hester_wc_add_percentage_to_sale_badge', 20, 3 );

				// Out of stock product badge.
				add_action( 'woocommerce_before_shop_loop_item_title', 'hester_wc_out_of_stock_badge', 10 );
				add_action( 'woocommerce_before_single_product_summary', 'hester_wc_out_of_stock_badge', 10 );
				...
				add_action( 'woocommerce_widget_shopping_cart_buttons', 'hester_wc_widget_shopping_cart_buttons', 10 );
				...
				add_filter( 'woocommerce_subcategory_count_html', 'hester_wc_cat_count_filter', 10, 2 );
				add_filter( 'woocommerce_rating_filter_count', 'hester_wc_rating_count_filter', 10, 3 );
				add_filter( 'woocommerce_layered_nav_count', 'hester_wc_layered_count_filter', 10, 3 );
			...
			// Enqueue styles.
			add_action( 'hester_enqueue_scripts', array( $this, 'enqueue' ) );
			...
			// Add correct sidebar.
			add_filter( 'hester_sidebar_name', array( $this, 'set_sidebar' ) );

			// Set sidebar position.
			add_filter( 'hester_default_sidebar_position', array( $this, 'set_default_sidebar_position' ) );

			// Remove item from cart.
			add_action( 'wp_ajax_hester_remove_wc_cart_item', array( $this, 'remove_item_from_cart' ) );
			add_action( 'wp_ajax_nopriv_hester_remove_wc_cart_item', array( $this, 'remove_item_from_cart' ) );

			// Add theme supports.
			add_action( 'after_setup_theme', array( $this, 'theme_supports' ), 20 );

			// Add customizer cart widget.
			add_filter( 'hester_customizer_widgets', array( $this, 'add_customizer_cart_widget' ) );
			add_filter( 'hester_main_header_widgets', array( $this, 'add_cart_to_main_header_widgets' ) );
			...
			// Add dynamic CSS.
			add_filter( 'hester_dynamic_styles', array( $this, 'dynamic_css' ), 5 );

			// Update dynamic styles on deactivation.
			add_action( 'deactivate_woocommerce/woocommerce.php', array( hester_dynamic_styles(), 'delete_dynamic_file' ) );

			// Return Shop page ID.
			add_filter( 'hester_get_the_id', array( $this, 'get_the_id' ) );
		...
		public function theme_supports() {
			...
			if ( hester_option( 'wc_product_gallery_zoom' ) ) {
				add_theme_support( 'wc-product-gallery-zoom' );
			}

			// Product Gallery Lightbox.
			if ( hester_option( 'wc_product_gallery_lightbox' ) ) {
		...
		public function enqueue() {
			...
			wp_enqueue_style(
				'hester-woocommerce',
				HESTER_THEME_URI . '/assets/css/compatibility/woocommerce' . $hester_suffix . '.css',
				false,
			...
			wp_enqueue_script(
				'hester-wc',
				HESTER_THEME_URI . '/assets/js/' . $hester_dir . 'hester-wc' . $hester_suffix . '.js',
				array( 'jquery' ),
				HESTER_THEME_VERSION,
				true
			);
		...
		public function product_catalog_elements() {
			...
			$elements = hester_option( 'product_catalog_elements' );
		...
		public function template_loop_category() {
			...
			if ( is_array( $product_cats ) && ! empty( $product_cats ) ) {
				foreach ( $product_cats as $product_cat ) {
					$cats[] = '<a class="hester-loop-product__category" href="' . esc_url( get_term_link( $product_cat, 'product_cat' ) ) . '">' . esc_html( $product_cat->name ) . '</a>';
				}
			}

			echo '<span class="hester-loop-product__category-wrap">' . wp_kses_post( implode( ', ', $cats ) ) . '</span>';
		}
		...
		public function result_wrapper_start() {
			...
			echo '<div class="hester-woo-before-shop clearfix">';
		}
		...
		public function cart_widget_count_fragment( $fragments ) {
			...
			$fragments['.hester-header-widget__cart a.hester-cart'] = hester_wc_cart_icon( false );
		...
		public function cart_widget_dropdown_fragment( $fragments ) {
			...
			$fragments['.hester-header-widget__cart .dropdown-item'] = hester_wc_cart_dropdown( false );
		...
		public function content_wrapper_end() {
			?>
				...
				<?php do_action( 'hester_woocommerce_sidebar' ); ?>
		...
		public function single_product_wrapper_start() {
			echo '<div class="hester-wc-product-wrap">';
		}
		...
		public function single_product_wrapper_end() {
			echo '</div><!-- END .hester-wc-product-wrap -->';
		}
		...
		public function product_actions() {
			...
			// Remove Hester page title on WooCommerce product pages.
			add_filter( 'hester_page_header_has_title', '__return_false' );

			// Disable Comments toggle.
			add_filter( 'hester_display_comments_toggle', '__return_false' );
		}
		...
		public function add_customizer_cart_widget( $widgets ) {
			...
			$widgets['cart'] = 'Hester_Customizer_Widget_Cart';
		...
		public function register_wc_sidebars() {
			...
			register_sidebar(
				apply_filters(
					'hester_woocommerce_sidebar_name',
					array(
						'name'          => esc_html__( 'WooCommerce Sidebar', 'hester' ),
						'id'            => 'hester-wc-sidebar',
						'description'   => __( 'Widgets in this area are displayed on WooCommerce pages except Product pages.', 'hester' ),
						'before_widget' => '<div id="%1$s" class="hester-sidebar-widget hester-widget widget %2$s">',
						'after_widget'  => '</div>',
						'before_title'  => '<h4 class="widget-title">' . $title_shape . '',
						'after_title'   => '</h4>',
					)
				)
			);
			...
			register_sidebar(
				apply_filters(
					'hester_woocommerce_product_sidebar_name',
					array(
						'name'          => esc_html__( 'Product Sidebar', 'hester' ),
						'id'            => 'hester-wc-product-sidebar',
						'description'   => __( 'Widgets in this area are displayed on WooCommerce Product pages.', 'hester' ),
						'before_widget' => '<div id="%1$s" class="hester-sidebar-widget hester-widget widget %2$s">',
						'after_widget'  => '</div>',
						'before_title'  => '<h4 class="widget-title">' . $title_shape . '',
						'after_title'   => '</h4>',
		...
		public function set_sidebar( $sidebar_name ) {
			...
			if ( is_product() ) {
				$sidebar_name = 'hester-wc-product-sidebar';
			} elseif ( is_woocommerce() || is_cart() || is_checkout() ) {
				$sidebar_name = 'hester-wc-sidebar';
			}
		...
		public function set_default_sidebar_position( $position ) {
			...
			if ( is_product() ) {
				$position = hester_option( 'wc_product_sidebar_position' );
			} elseif ( is_woocommerce() || is_cart() || is_checkout() ) {
				$position = hester_option( 'wc_sidebar_position' );
			}

			if ( is_product() || is_woocommerce() || is_cart() || is_checkout() ) {
				if ( 'default' === $position ) {
					return hester_option( 'sidebar_position' );
				} else {
					return $position;
				}
			}
		...
		/**
		 * Remove Hester page description on WooCommerce pages.
		 *
		 * @since 1.0.0
		...
		public function remove_item_from_cart() {
			...
			check_ajax_referer( 'hester-nonce' );
		...
		public function product_image_swap() {
			...
			$hover_style = hester_option( 'shop_product_hover' );
			...
			if ( 'image-swap' === $hover_style ) {
				...
				if ( $attachment_ids ) {
					...
					echo wp_kses_post( apply_filters( 'hester_woocommerce_product_image_swap', wp_get_attachment_image( $attachment_id, $image_size, false, array( 'class' => 'show-on-hover' ) ) ) );
				}
			}
		}
		...
		public function loop_product_thumb_wrap_start() {
			...
			$class = 'hester-product-thumb';

			if ( 'image-swap' === hester_option( 'shop_product_hover' ) ) {
		...
		public function loop_product_thumb_wrap_end() {
			echo '</div><!-- END .hester-product-thumb -->';
		}
		...
		public function load_customizer_widget() {
			...
			$path = HESTER_THEME_PATH . '/inc/compatibility/woocommerce/class-hester-customizer-widget-cart.php';
		...
		public function review_order_heading() {
			...
			echo wp_kses_post( apply_filters( 'hester_review_order_heading', '<h3>' . __( 'Payment', 'hester' ) . '</h3>' ) );
		}
		...
		public function checkout_shipping_heading() {
			...
			echo wp_kses_post( apply_filters( 'hester_checkout_shipping_heading', '<h3>' . __( 'Shipping', 'hester' ) . '</h3>' ) );
		}
		...
		public function single_product_related_products_args( $args ) {
			...
			$columns = intval( hester_option( 'wc_related_columns' ) );
			$rows    = intval( hester_option( 'wc_related_rows' ) );
		...
		public function woocommerce_cross_sell_display() {
			...
			if ( hester_option( 'wc_cross_sell_products' ) ) {

				$rows = intval( hester_option( 'wc_cross_sell_rows' ) );
		...
		public function woocommerce_upsell_display() {
			...
			if ( hester_option( 'wc_upsell_products' ) ) {

				$columns = intval( hester_option( 'wc_upsells_columns' ) );
				$rows    = intval( hester_option( 'wc_upsells_rows' ) );
		...
		public function woocommerce_related_products() {
			...
			if ( hester_option( 'wc_related_products' ) ) {
		...
		public function single_product_slider_options( $options ) {
			...
			if ( ! hester_option( 'wc_product_slider_arrows' ) ) {
				return $options;
			}

			$options['directionNav'] = true;
			$options['prevText']     = hester_animated_arrow( 'left', false );
			$options['nextText']     = hester_animated_arrow( 'right', false );
		...
		public function related_products_heading( $heading ) {
			return __( 'Related Products', 'hester' );
		}
		...
		public function admin_redirects() {
			...
			if ( false !== strpos( $current_page, 'hester' ) ) {
		...
		public function dynamic_css( $css ) {
			...
			$css .= '
				...
				.woocommerce div.product .woocommerce-tabs ul.tabs li a {
					color: ' . hester_hex2rgba( $content_text_color, 0.73 ) . ';
				}
			...
			$css .= '
				...
				.woocommerce .quantity .hester-woo-minus,
				.woocommerce .quantity .hester-woo-plus {
			...
			// Background.
			$header_background = hester_option( 'header_background' );

			if ( 'color' === $header_background['background-type'] && $header_background['background-color'] ) {
				$css .= '
					.hester-header-widget__cart .hester-cart .hester-cart-count { 
			...
			// Headings.
			$css .= hester_dynamic_styles()->get_typography_field_css( '.woocommerce div.product h1.product_title, .woocommerce #reviews #comments h2, .woocommerce .cart_totals h2, .woocommerce .cross-sells > h4, .woocommerce #reviews #respond .comment-reply-title', 'headings_font' );

			$css .= hester_dynamic_styles()->get_typography_field_css( '.woocommerce div.product h1.product_title', 'h2_font' );
			$css .= hester_dynamic_styles()->get_typography_field_css( '.woocommerce #reviews #comments h2', 'h3_font' );
			$css .= hester_dynamic_styles()->get_typography_field_css( '.woocommerce .cart_totals h2, .woocommerce .cross-sells > h4, .woocommerce #reviews #respond .comment-reply-title', 'h4_font' );
...
if ( ! function_exists( 'hester_woocommerce' ) ) :
	/**
	 * The function which returns the one Hester_Woocommerce instance.
	 *
	 * @since 1.0.0
	 * @return object
	 */
	function hester_woocommerce() {
		return Hester_Woocommerce::instance();
	}
endif;
