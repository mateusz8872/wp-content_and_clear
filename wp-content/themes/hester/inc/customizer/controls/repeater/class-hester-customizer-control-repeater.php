...
if ( ! class_exists( 'Hester_Customizer_Control_Repeater' ) ) :

	/**
	 * Customizer Repeatable control class.
	 */
	class Hester_Customizer_Control_Repeater extends Hester_Customizer_Control {
		/**
		...
		public function __construct( $manager, $id, $args = array() ) {
			...
			if ( ! isset( $args['allow_unlimited'] ) || false !== $args['allow_unlimited'] ) {
				$this->max_item = apply_filters( 'hester_reepeatable_max_item', absint( $args['max_item'] ) );
			} else {
				$this->max_item = absint( $args['max_item'] );
			}
			$this->changeable          = isset( $args['changeable'] ) && 'no' === $args['changeable'] ? 'no' : 'yes';
			$this->default_empty_title = isset( $args['default_empty_title'] ) && '' !== $args['default_empty_title'] ? $args['default_empty_title'] : esc_html__( 'Item', 'hester' );

			add_action( 'customize_controls_print_footer_scripts', array( __CLASS__, 'item_tpl' ), 66 );
			add_action( 'customize_controls_enqueue_scripts', array( $this, 'hester_customize_controls_enqueue_scripts' ) );
		}
		...
		public function to_json() {
			...
			$this->json['add_text']            = $this->add_text ?? __( 'Add new item', 'hester' );
		...
		public function enqueue() {
			...
			if ( in_array( 'button_set', array_values( array_column( $this->fields, 'type' ) ), true ) ) {
				wp_enqueue_style(
					'hester-background-alignment-style',
					HESTER_THEME_URI . '/inc/customizer/controls/alignment/alignment' . $hester_suffix . '.css',
					false,
		...
		<# if ( data.label ) {   #>
			<div class="hester-control-heading customize-control-title hester-field hester-control-wrapper">
			...
			<div class="widget">
				<div class="widget-top">
					...
					<div class="widget-title">
						<h4 class="live-title"><?php esc_html_e( 'Item', 'hester' ); ?></h4>
					</div>
				</div> 
									...
									#>
										<# if ( field.type !== 'checkbox' ) { #>
											<# if ( field.title && field.type != 'design-options' ) { #>
												<div class="hester-control-heading hester-control-wrapper">
													<div class="customize-control-title">
														...
														<# if ( field.desc ) { #>
															<i class="hester-info-icon">
																...
																<span class="hester-tooltip">{{{ field.desc }}}</span>
															</i>
														<# } #>
													</div>
												</div>
										...
										<# } else if ( field.type === 'button_set' ) { #>
											<div class="hester-alignment-control button_set">
												<div class="button-group hester-middle">
											<# for ( k in field.options ) { #>
												<# if ( field.options.hasOwnProperty( k ) ) { #>
													<label class="hester-{{ k }}">
										...
										<# } else if ( field.type == 'media' ) { #>
											...
											<# } #>
											<div class="actions">
												<button class="button remove-button " <# if ( ! field.value.url ){ #> style="display:none"; <# } #> type="button"><?php esc_html_e( 'Remove', 'hester' ); ?></button>
												<button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add', 'hester' ); ?>" data-change-txt="<?php esc_attr_e( 'Change', 'hester' ); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php esc_html_e( 'Add', 'hester' ); ?> <# } else { #> <?php esc_html_e( 'Change', 'hester' ); ?> <# } #> </button>
										...
										<# } else if ( field.type == 'icon'  ) { #>
											...
											<a href="#" class="remove-icon"><?php esc_html_e( 'Remove', 'hester' ); ?></a>
											<# } else if(field.type == 'gradient'){ #>
												...
												<div data-dep-field="background-type" data-dep-value="gradient">
													...
													<div class="popup-element color-element style-1">
														<label for="gradient-color-1-{{ field.id }}"><?php esc_html_e( 'Color 1', 'hester' ); ?></label>
													...
													<div class="hester-range-wrapper popup-element color-element style-2" data-option-id="gradient-color-1-location">
														<label for="gradient-color-1-location-{{ field.id }}"><?php esc_html_e( 'Color location', 'hester' ); ?></label>
													...
													<!-- Color 2 -->
													<div class="popup-element color-element style-1">
														<label for="gradient-color-2-{{ field.id }}"><?php esc_html_e( 'Color 2', 'hester' ); ?></label>
													...
													<div class="hester-range-wrapper popup-element color-element style-2" data-option-id="gradient-color-2-location">
														<label for="gradient-color-2-location-{{ field.id }}"><?php esc_html_e( 'Color location', 'hester' ); ?></label>
													...
													<div class="hester-select-wrapper popup-element style-1">
														<label for="gradient-type-{{ field.id }}"><?php esc_html_e( 'Gradient type', 'hester' ); ?></label>
														<div class="popup-input-wrapper">
															<select data-option="gradient-type" id="gradient-type-{{ field.id }}" data-repeat-name="_items[__i__][{{ field.id }}][gradient-type]">
																<option value="linear"<# if ( 'linear' === field.value['gradient-type'] ) { #> selected="selected"<# } #>><?php esc_html_e( 'Linear', 'hester' ); ?></option>
																<option value="radial"<# if ( 'radial' === field.value['gradient-type'] ) { #> selected="selected"<# } #>><?php esc_html_e( 'Radial', 'hester' ); ?></option>
															</select>
														</div>
													</div>
													...
													<div data-dep-field="gradient-type" data-dep-value="linear" class="hester-range-wrapper popup-element color-element style-2" data-option-id="gradient-linear-angle">
														<label for="gradient-angle-{{ field.id }}"><?php esc_html_e( 'Angle', 'hester' ); ?></label>
													...
													<div data-dep-field="gradient-type" data-dep-value="radial" class="hester-select-wrapper popup-element style-1">
														<label for="gradient-position-{{ field.id }}"><?php esc_html_e( 'Position', 'hester' ); ?></label>
														<div class="popup-input-wrapper">
															<!-- <# var choices = 
												...
												<# } else if(field.type == 'design-options') { #> 
													...
													<div class="hidden popup-content">
														...
														<# if ( 'background' in field.display ) { #>
															...
															<!-- Background Type -->
															<div class="hester-select-wrapper popup-element style-1">
																<label for="background-type-{{ field.id }}"><?php esc_html_e( 'Background type', 'hester' ); ?></label>
																<div class="popup-input-wrapper">
															...
															<# if ( 'color' in field.display['background'] ) { #>
																...
																<div data-dep-field="background-type" data-dep-value="color">
																	<!-- Background Color -->
																	<div class="popup-element color-element style-1">
																		<label for="background-color-{{ field.id }}"><?php esc_html_e( 'Background color', 'hester' ); ?></label>
															...
															<# if ( 'gradient' in field.display['background'] ) { #>
																...
																<div data-dep-field="background-type" data-dep-value="gradient">
																	...
																	<!-- Radial Position -->
																	<div data-dep-field="gradient-type" data-dep-value="radial" class="hester-select-wrapper popup-element style-1">
																		<label for="gradient-position-{{ field.id }}"><?php esc_html_e( 'Gradinet position', 'hester' ); ?></label>
																		<div class="popup-input-wrapper">
																			<?php
																				$choices = array(
																					'center center' => esc_html__( 'Center Center', 'hester' ),
																	...
																	<div class="background-image">
																		...
																		<div class="attachment-media-view background-image-upload">
																			...
																			<div class="actions">
																				...
																				<button class="button background-image-upload-remove-button<# if ( ! field.value['background-image'] ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'hester' ); ?></button>

																				<button type="button" class="button background-image-upload-button"><?php esc_html_e( 'Select image', 'hester' ); ?></button>
																	...
																	<div class="background-image-advanced">
																		...
																		<div class="background-repeat">
																			<select {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-option="background-repeat" data-repeat-name="_items[__i__][{{ field.id }}][background-repeat]">
																				<option value="no-repeat"<# if ( 'no-repeat' === field.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'No Repeat', 'hester' ); ?></option>
																				<option value="repeat"<# if ( 'repeat' === field.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat All', 'hester' ); ?></option>
																				<option value="repeat-x"<# if ( 'repeat-x' === field.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat Horizontally', 'hester' ); ?></option>
																				<option value="repeat-y"<# if ( 'repeat-y' === field.value['background-repeat'] ) { #> selected <# } #>><?php esc_html_e( 'Repeat Vertically', 'hester' ); ?></option>
																			</select>
																		</div>
																		...
																		<div class="background-size">
																			...
																			<div class="buttonset">
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-size]" data-option="background-size" class="switch-input screen-reader-text" type="radio" value="cover" id="{{ field.id }}cover" <# if ( 'cover' === field.value['background-size'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}cover"><?php esc_html_e( 'Cover', 'hester' ); ?></label>
																				</input>
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-size]" data-option="background-size" class="switch-input screen-reader-text" type="radio" value="contain" id="{{ field.id }}contain" <# if ( 'contain' === field.value['background-size'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}contain"><?php esc_html_e( 'Contain', 'hester' ); ?></label>
																				</input>
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-size]" data-option="background-size" class="switch-input screen-reader-text" type="radio" value="auto" id="{{ field.id }}auto" <# if ( 'auto' === field.value['background-size'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}auto"><?php esc_html_e( 'Auto', 'hester' ); ?></label>
																				</input>
																			</div>
																		</div>
																		...
																		<div class="background-attachment">
																			...
																			<div class="buttonset">
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-attachment]" data-option="background-attachment" lass="switch-input screen-reader-text" type="radio" value="inherit" id="{{ field.id }}inherit" <# if ( 'inherit' === field.value['background-attachment'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}inherit"><?php esc_html_e( 'Inherit', 'hester' ); ?></label>
																				</input>
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-attachment]" data-option="background-attachment" class="switch-input screen-reader-text" type="radio" value="scroll" id="{{ field.id }}scroll" <# if ( 'scroll' === field.value['background-attachment'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}scroll"><?php esc_html_e( 'Scroll', 'hester' ); ?></label>
																				</input>
																				<input {{{ data.inputAttrs }}} data-live-id="{{{ field.id }}}" data-repeat-name="_items[__i__][{{ field.id }}][background-attachment]" data-option="background-attachment" class="switch-input screen-reader-text" type="radio" value="fixed" id="{{ field.id }}fixed" <# if ( 'fixed' === field.value['background-attachment'] ) { #> checked="checked" <# } #>>
																					<label class="switch-label" for="{{ field.id }}fixed"><?php esc_html_e( 'Fixed', 'hester' ); ?></label>
																				</input>
																			</div>
																		</div>
																		...
																		<div class="background-color-overlay popup-element color-element style-1">
																			...
																			<label for="background-color-overlay-{{ field.id }}"><h4><?php esc_html_e( 'Overlay Color', 'hester' ); ?></h4></label>
																		...
																		<ul class="active">
																			...
																			<li class="spacing-control-wrap">
																				<div class="spacing-link-values">
																					<span class="dashicons dashicons-admin-links hester-spacing-linked" data-element="{{ field.id }}" title="{{ data.title }}"></span>
																					<span class="dashicons dashicons-editor-unlink hester-spacing-unlinked" data-element="{{ field.id }}" title="{{ data.title }}"></span>
																				</div>
																			</li>
							...
							<div class="widget-control-actions">
								<div class="alignleft">
									<span class="remove-btn-wrapper">
										<a href="#" class="repeat-control-remove" title=""><?php esc_html_e( 'Remove', 'hester' ); ?></a> |
									</span>
									<a href="#" class="repeat-control-close"><?php esc_html_e( 'Close', 'hester' ); ?></a>
								</div>
								<br class="clear">
							</div>
						</div>
		 ...
		 */
		public function hester_customize_controls_enqueue_scripts() {
			wp_localize_script(
				'customize-controls',
				'Hester_Icon_Picker',
				apply_filters(
					'hester_icon_picker_js_setup',
					array(
						'search' => esc_html__( 'Search', 'hester' ),
						'fonts'  => array(
							'font-awesome' => array(
								// Name of icon.
								'name'   => esc_html__( 'Font Awesome', 'hester' ),
