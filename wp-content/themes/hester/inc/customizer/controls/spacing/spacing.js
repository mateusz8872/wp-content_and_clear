;(function($) {
 	...
 	wp.customize.controlConstructor['hester-spacing'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			// Change unit.
			control.container.find( '.hester-control-unit input[type="radio"]' ).on( 'change', function() {
				control.save_value();
			});
		...
		save_value: function() {
			...
			var new_val = {},
				...
				choice,
				device,
				units   = this.container.find( '.hester-control-unit' );
