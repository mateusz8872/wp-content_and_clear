;(function( $ ) {
	...
	wp.customize.controlConstructor['hester-button'] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click', '.button', function(e){
				...
				if ( action ) {
					...
					var data = {
						_ajax_nonce: hester_customizer_localized.wpnonce,
						action: action,
					};

			 		$.post( hester_customizer_localized.ajaxurl, data, function(response) {
