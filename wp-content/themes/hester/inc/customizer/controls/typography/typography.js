;(function($) {
 	...
 	// Typography control
	wp.customize.controlConstructor[ 'hester-typography' ] = wp.customize.Control.extend({
		...
		ready: function() {
			...
			control.container.find( '.reset-defaults' ).on( 'click', function(){
				...
				rangeFields.forEach( (item) => {
					control.container.find( '[data-option-id="' + item + '"]' ).find( '.hester-reset-range' ).click();
				});
			});
		...
		update_font_weight_field: function( font_family ) {
			...
			if ( 'inherit' === font_family || 'default' === font_family ) {
				...
				$.each( hester_typography_vars.fonts, function( group_id, group ){
			...
			self.container.find( '.hester-typography-font-weight' ).find( 'select' ).html( options );
		},
		...
		update_font_subsets_field: function( font_family ) {
			...
			var self     = this,
				...
				selected = '',
				setting  = self.setting.get(),
				field    = self.container.find( '.hester-typography-font-subsets' );
