;( function( $ ) {
	...
	wp.customize.controlConstructor['hester-range'] = wp.customize.Control.extend({
	   ...
	   ready: function() {
		   ...
		   control.container.find( '.hester-range-wrapper' ).rangeControl({
	   ...
	   save: function() {
		   ...
		   if ( false === devices || devices === undefined || 0 == devices.length ) {
			   value.value = parseFloat( this.container.find( '.hester-range-input' ).val() );
		   } else {
			   for ( var device in devices ) {
				   value[ device ] = parseFloat( this.container.find( '.control-responsive.' + device + ' .hester-range-input' ).val() );
			   }
		   }

		   if ( this.container.find( '.hester-control-unit' ).length ) {
			   value.unit = this.container.find( '.hester-control-unit input:checked' ).val();
		   }
   ...
   ( function( $, undef ) {
	   ...
	   RangeControl = {
		   ...
		   _create: function() {
			   ...
			   self.unit = el.find( '.hester-control-wrap' ).attr( 'data-unit' );
		   ...
		   _addListeners: function() {
				...
				* @since 1.0.0
				*/
			   self.element.on( 'keyup input paste', '.hester-range-input', function() {
			   ...
			   self.element.on( 'click', 'input[type=range]', function() {
				   ...
				   range = $( this );
				   input = range.parent().children( '.hester-range-input' );
				...
				* @since 1.0.0
				*/
			   self.element.on( 'click', '.hester-reset-range', function() {

				   var $el;

				   self.element.find( '.hester-reset-range' ).each( function( index, el ) {

					   $el = $( el );

					   // Reset range unit.
					   self.unit = $el.data( 'reset_unit' );
					   $el.closest( '.hester-range-wrapper' ).find( '.hester-control-unit input[type="radio"][value="' + $el.data( 'reset_unit' ) + '"]' ).click();
				...
				* @since 1.0.0
				*/
			   self.element.on( 'click', '.hester-control-unit input', function() {

				   var $unit = $( this );

				   self.element.find( '.hester-control-wrap input[type="range"]' ).each( function( index, el ) {
				   ...
				   if ( 'px' === $unit.val() && ( 'rem' === self.unit || 'em' === self.unit ) ) {
					   ...
					   self.element.find( 'input.hester-range-input' ).each( function( index, el ) {
		   ...
		   _initResponsiveControls: function() {
			   ...
			   var self = this,
				   el = self.element,
				   wrap = el.find( '.hester-control-wrap' );

			   el.addClass( 'hester-control-responsive' );

			   // Populate responsive switcher.
			   if ( el.find( '.customize-control-title' ).length ) {

				   var $switcher = $( '<ul class="hester-responsive-switchers"></ul>' );

				   for ( var device in self.options.responsive ) {
					   $switcher.append( '<li class="' + device + '"><span class="preview-' + device + '" data-device="' + device + '"><span class="hester-tooltip small-tooltip">' + device + '</span><i class="' + self.options.responsive[device].icon + '"></i></span></li>' );
				   }
		   ...
		   _initUnit: function() {
			   ...
			   var template = wp.template( 'hester-control-unit' );
			   ...
			   if ( 'object' === typeof self.options.unit ) {
				   ...
				   $( '<span class="hester-range-unit">' + self.options.unit + '</span>' ).insertAfter( wrap.find( '.hester-range-input' ) );
			   }

			   wrap.parent().find( '.hester-control-unit input:checked' ).click();
		   }
	   };

	   // Register the color picker as a widget.
	   $.widget( 'hester.rangeControl', RangeControl );
   }( jQuery ) );
