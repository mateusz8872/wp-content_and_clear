 ...
 */
function hester_sanitize_number( $val ) {
 ...
 */
function hester_sanitize_toggle( $input, $setting ) {

	if ( 'hester_enable_front_page' === $setting ) {
		sanitize_key( $input );
	}
 ...
 * @return boolean
 */
function hester_sanitize_checkbox( $input ) {
 ...
 * @param object $setting  Setting object.
 */
function hester_sanitize_select( $input, $setting ) {
 ...
 * @param object $setting  Setting object.
 */
function hester_sanitize_textarea( $input, $setting ) {
 ...
 */
function hester_sanitize_color( $color ) {
	...
	if ( false === strpos( $color, 'rgba' ) ) {
		return hester_sanitize_hex_color( $color );
	}

	return hester_sanitize_alpha_color( $color );
}
 ...
 */
function hester_sanitize_hex_color( $color ) {
 ...
 */
function hester_sanitize_alpha_color( $color ) {
 ...
 * @param object $setting  Setting object.
 */
function hester_sanitize_image( $image, $setting ) {
	/*
 ...
 */
function hester_sanitize_widget( $widgets, $setting ) {
 ...
 */
function hester_no_sanitize( $val ) {
	return $val;
}
 ...
 * @return string
 */
function hester_sanitize_order( $input ) {
	...
	$filter_empty = array_filter( $json, 'hester_not_empty' );
 ...
 */
function hester_sanitize_background( $background ) {
 ...
 */
function hester_sanitize_spacing( $values, $setting, $default = array() ) {
 ...
 * @param mixed  $default Default Value.
 */
function hester_sanitize_range( $value, $setting, $default = '' ) {
 ...
 * @param object $setting WP Customize Setting instance.
 */
function hester_sanitize_responsive( $values, $setting ) {

	$control       = $setting->manager->get_control( $setting->id );
	$control_type  = str_replace( 'hester-', '', $control->type );
 ...
 * @param object $setting  Setting object.
 */
function hester_sanitize_typography( $value, $setting ) {
 ...
 * @param object $setting  Setting object.
 */
function hester_sanitize_design_options( $value, $setting ) {
	...
	if ( isset( $control->display ) ) {
		...
		if ( isset( $control->display['background'] ) ) {
			...
			if ( array_key_exists( 'color', $control->display['background'] ) ) {
				...
				if ( isset( $value['background-color'] ) ) {
					$sanitized['background-color'] = hester_sanitize_color( $value['background-color'] );
				}
			}
			...
			if ( array_key_exists( 'gradient', $control->display['background'] ) ) {
				...
				if ( isset( $value['gradient-color-1'] ) ) {
					$sanitized['gradient-color-1'] = hester_sanitize_color( $value['gradient-color-1'] );
				}

				if ( isset( $value['gradient-color-2'] ) ) {
					$sanitized['gradient-color-2'] = hester_sanitize_color( $value['gradient-color-2'] );
				}

				if ( isset( $value['gradient-color-1-location'] ) ) {
					$sanitized['gradient-color-1-location'] = hester_sanitize_range( $value['gradient-color-1-location'], $setting );
				}

				if ( isset( $value['gradient-color-2-location'] ) ) {
					$sanitized['gradient-color-2-location'] = hester_sanitize_range( $value['gradient-color-2-location'], $setting );
				}
				...
				if ( isset( $value['gradient-linear-angle'] ) ) {
					$sanitized['gradient-linear-angle'] = hester_sanitize_range( $value['gradient-linear-angle'], $setting );
				}
		...
		if ( isset( $control->display['color'] ) ) {
			foreach ( $control->display['color'] as $id => $title ) {
				if ( isset( $value[ $id ] ) ) {
					$sanitized[ $id ] = hester_sanitize_color( $value[ $id ] );
				}
			}
		}
		...
		if ( isset( $control->display['border'] ) ) {
			...
			if ( array_key_exists( 'color', $control->display['border'] ) && isset( $value['border-color'] ) ) {
				$sanitized['border-color'] = hester_sanitize_color( $value['border-color'] );
			}
			...
			if ( isset( $value['separator-color'] ) ) {
				$sanitized['separator-color'] = hester_sanitize_color( $value['separator-color'] );
			}
 ...
 * @param object $setting  Setting object.
 */
function hester_sanitize_sortable( $value, $setting ) {
 ...
 */
function hester_repeater_sanitize( $input, $setting ) {
	$control = $setting->manager->get_control( $setting->id );
			...
			if ( isset( $fields[ $id ] ) ) {
				switch ( strtolower( $fields[ $id ]['type'] ) ) {
					...
					case 'editor':
						...
						break;
					case 'color':
						$data[ $i ][ $id ] = hester_sanitize_hex_color( $value );
						break;
					case 'coloralpha':
						$data[ $i ][ $id ] = hester_sanitize_alpha_color( $value );
						break;
					case 'checkbox':
						$data[ $i ][ $id ] = hester_sanitize_checkbox( $value );
						break;
					case 'select':
						$data[ $i ][ $id ] = '';
					...
					case 'gradient':
						...
						break;
					case 'design-options':
						hester_sanitize_design_options( $value, $setting );
						break;
					default:
						$data[ $i ][ $id ] = wp_kses_post( $value );
				}
