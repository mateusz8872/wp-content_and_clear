; ( function( api, $ ) {
	...
	// Extends our custom "hester-info" section. Make it visible.
	api.sectionConstructor['hester-info'] = api.Section.extend({
	...
	// Store old previewer.
	var hesterOldPreviewer = api.Previewer;
	api.Previewer = hesterOldPreviewer.extend({
		initialize: function( params, options ) {

			// Store a reference to the Previewer
			api.hesterCustomizerPreviewer.preview = this;

			// Call the old Previewer's initialize function
			hesterOldPreviewer.prototype.initialize.call( this, params, options );
		}
	});

	api( 'hester_info_style', function( setting ) {
		...
		setting.bind( function( value ) {
			if ( 12 == value ) {
				...
				api.control( 'hester_info_section_spacing' ).setting( data );

				// console.log( 'New value', api.control( 'hester_info_section_spacing' ).setting() );
			} else {
				var data = {
			...
			var selector = api.control( 'hester_info_section_spacing' ).selector;
	...
	_.each( hester_customizer_localized.preview_url_for_section, function( url, id ) { // eslint-disable-line camelcase
		if ( url ) {
	...
	$( document ).ready( function( $ ) {
		...
		api.hesterCustomizerPreviewer.init();

		// Display the first responsive control
		$( '.hester-control-responsive' ).each( function() {
			$( this ).find( '.control-responsive' ).first().addClass( 'active' );
		});

		// Responsive switchers
		$( '.customize-control' ).on( 'click', '.hester-responsive-switchers span', function( event ) {

			var $this = $( this ),
				$switcherContainer = $this.closest( '.hester-responsive-switchers' ),
				$switcherButtons = $switcherContainer.find( 'li span' ),
				$device = $( event.currentTarget ).data( 'device' ),
				$control = $( '.hester-control-responsive' ),
		...
		$( '.wp-full-overlay-footer .devices button' ).on( 'click', function( event ) {
			...
			// Set up variables
			var $this = $( this ),
				$devices = $( '.customize-control .hester-responsive-switchers' ),
				$device = $( event.currentTarget ).data( 'device' ),
				$control = $( '.hester-control-responsive' );
		...
		// Tooltip positioning
		if ( $( '.hester-tooltip' ).length ) {

			var $tooltip, $iconPosLeft, $iconPosRight, $titleWidth;

			$( '.hester-tooltip' ).each( function() {
				$tooltip = $( this );
				...
				$titleWidth = $tooltip.closest( '.hester-control-wrapper' ).outerWidth();

				$iconPosLeft = $tooltip.closest( '.hester-info-icon' ).css( 'position', 'static' ).position().left;
		...
		api.hasOwnProperty( 'previewer' ) && ( api.previewer.bind( 'hester-customize-disable-section', function( e ) {
			$( 'input[id=' + e + ']' ).trigger( 'click' ).trigger( 'change' );
			const val = ! api.control( e ).setting.get();
			api.control( e ).setting.set( val );
		}), api.previewer.bind( 'hester-customize-focus-control', function( e ) {
			api.control( e ).focus();
		}) );

		//Scroll to section
		$( 'body' ).on( 'click', '#sub-accordion-panel-hester_panel_homepage > .accordion-section.control-subsection', function( event ) {
