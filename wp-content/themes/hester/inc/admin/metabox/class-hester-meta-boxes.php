...
if ( ! class_exists( 'Hester_Meta_Boxes' ) ) :

	/**
	 * Meta Boxes Class.
	 */
	class Hester_Meta_Boxes {
		...
		public function metabox_config() {
			...
			// Sidebar position.
			$layout_settings['hester_sidebar_position'] = array(
				'id'      => 'hester_sidebar_position',
				'name'    => esc_html__( 'Sidebar', 'hester' ),
				'type'    => 'select',
				'default' => 'default',
				'choices' => array(
					''              => esc_html__( 'Default (from Customizer)', 'hester' ),
			...
			// Content layout.
			$layout_settings['hester_content_layout'] = array(
				'id'      => 'hester_content_layout',
				'name'    => esc_html__( 'Content Layout', 'hester' ),
				'type'    => 'select',
				'default' => 'default',
				'choices' => array(
					''                => esc_html__( 'Default (from Customizer)', 'hester' ),
			...
			// Transparent header.
			$layout_settings['hester_transparent_header'] = array(
				'id'      => 'hester_transparent_header',
				'name'    => esc_html__( 'Transparent Header', 'hester' ),
				'type'    => 'select',
				'default' => '',
				'choices' => array(
					''        => esc_html__( 'Default (from Customizer)', 'hester' ),
					'enable'  => esc_html__( 'Enable', 'hester' ),
					'disable' => esc_html__( 'Disable', 'hester' ),
				),
			);

			// Start checkbox group.
			$layout_settings['hester_page_sections'] = array(
				'id'     => 'hester_page_sections',
				'type'   => 'group',
				'fields' => array(),
			);

			// Disable Top Bar.
			if ( hester_option( 'top_bar_enable' ) ) {
				$layout_settings['hester_page_sections']['fields']['hester_disable_topbar'] = array(
					'id'      => 'hester_disable_topbar',
					'name'    => esc_html__( 'Disable Top Bar', 'hester' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			// Disable Header.
			$layout_settings['hester_page_sections']['fields']['hester_disable_header'] = array(
				'id'      => 'hester_disable_header',
				'name'    => esc_html__( 'Disable Main Header', 'hester' ),
				'default' => false,
				'type'    => 'checkbox',
			);
			...
			if ( hester_option( 'page_header_enable' ) ) {
				$layout_settings['hester_page_sections']['fields']['hester_disable_page_title'] = array(
					'id'      => 'hester_disable_page_title',
					'name'    => esc_html__( 'Disable Page Title', 'hester' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			if ( hester_option( 'page_header_enable' ) && hester_option( 'breadcrumbs_enable' ) && ! hester_is_section_disabled( hester_option( 'breadcrumbs_hide_on' ), get_the_ID() ) ) {
				$layout_settings['hester_page_sections']['fields']['hester_disable_breadcrumbs'] = array(
					'id'      => 'hester_disable_breadcrumbs',
					'name'    => esc_html__( 'Disable Breadcrumbs', 'hester' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			$layout_settings['hester_page_sections']['fields']['hester_disable_thumbnail'] = array(
				'id'      => 'hester_disable_thumbnail',
				'name'    => esc_html__( 'Disable Featured Image', 'hester' ),
				'default' => false,
				'type'    => 'checkbox',
			);

			// Disable Prefooter.
			if ( hester_option( 'enable_pre_footer_cta' ) && ! hester_is_section_disabled( hester_option( 'pre_footer_cta_hide_on' ), get_the_ID() ) ) {
				$layout_settings['hester_page_sections']['fields']['hester_disable_prefooter_cta'] = array(
					'id'      => 'hester_disable_prefooter_cta',
					'name'    => esc_html__( 'Disable Pre Footer CTA', 'hester' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			if ( hester_option( 'enable_footer' ) ) {
				$layout_settings['hester_page_sections']['fields']['hester_disable_footer'] = array(
					'id'      => 'hester_disable_footer',
					'name'    => esc_html__( 'Disable Main Footer', 'hester' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			if ( hester_option( 'enable_copyright' ) ) {
				$layout_settings['hester_page_sections']['fields']['hester_disable_copyright'] = array(
					'id'      => 'hester_disable_copyright',
					'name'    => esc_html__( 'Disable Copyright Bar', 'hester' ),
					'default' => false,
					'type'    => 'checkbox',
				);
			}
			...
			// Set metabox options.
			$this->metabox = array(
				'hester-theme-page-options' => array(
					'id'         => 'hester-theme-page-options',
					'post_type'  => $this->get_post_types(),
					'capability' => 'edit_posts',
					'label'      => esc_html__( 'Theme Page Options', 'hester' ),
					'context'    => 'side',
					'priority'   => 'core',
			...
			$this->metabox = apply_filters( 'hester_metabox_options', $this->metabox );
		}
		...
		public function render( $post, $metabox ) {
			...
			foreach ( $metabox['args']['settings'] as $setting ) {
				...
				echo '<div class="hester-meta-box-field">';
		...
		public function render_field_checkbox( $post, $setting ) {
			...
			?>
			<div class="components-panel__row hester-checkbox-row">
				<div class="components-base-control">
					<div class="components-base-control__field">
						<span class="components-checkbox-control__input-container hester-checkbox-wrapper">
							...
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" role="img" class="hester-checked-icon components-checkbox-control__checked" aria-hidden="true" focusable="false"><path d="M9 18.6L3.5 13l1-1L9 16.4l9.5-9.9 1 1z"></path></svg>
						</span>
		...
		public function load_assets( $hook ) {
			...
			if ( 'post-new.php' === $hook || 'post.php' === $hook ) {
				...
				if ( in_array( $post_type, $post_types, true ) ) {
					...
					wp_enqueue_style(
						'hester-metabox-style',
						HESTER_THEME_URI . '/inc/admin/assets/css/hester-meta-boxes' . $hester_suffix . '.css',
						false,
...
/**
 * The function which returns the one Hester Meta Boxes instance.
 *
 ...
 * @return object
 */
function hester_meta_boxes() {
	return Hester_Meta_Boxes::get_instance();
}
