...
if ( ! function_exists( 'hester_breadcrumb_trail' ) ) {
	 ...
	 */
	function hester_breadcrumb_trail( $args = array() ) {
		...
		// Hester breadcrumb trail.
		$breadcrumb = apply_filters( 'hester_breadcrumb_trail_object', null, $args );

		if ( ! is_object( $breadcrumb ) ) {
			$breadcrumb = new Hester_Breadcrumb_Trail( $args );
		}
	 ...
	 * @since 1.0.0
	 */
	function hester_seopress_breadcrumbs_before_html() {
		echo '<div class="hester-breadcrumbs-wrapper">';
	}
	add_action( 'seopress_breadcrumbs_before_html', 'hester_seopress_breadcrumbs_before_html' );
	 ...
	 * @since 1.0.0
	 */
	function hester_seopress_breadcrumbs_after_html() {
		echo '</div>';
	}
	add_action( 'seopress_breadcrumbs_after_html', 'hester_seopress_breadcrumbs_after_html' );
	 ...
	 */
	function hester_breadcrumb_trail_items( $items, $args ) {
	...
	add_filter( 'hester_breadcrumb_trail_items', 'hester_breadcrumb_trail_items', 10, 2 );
}
...
class Hester_Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'hester_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		$breadcrumb = apply_filters( 'hester_breadcrumb_trail', $breadcrumb, $this->args );
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:', 'hester' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'hester' ),
			'home'                => esc_html__( 'Home', 'hester' ),
			'error_404'           => esc_html__( '404 Not Found', 'hester' ),
			'archives'            => esc_html__( 'Archives', 'hester' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s', 'hester' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'hester' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s', 'hester' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'hester' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'hester' ),
		...
		$this->labels = apply_filters( 'hester_breadcrumb_trail_labels', wp_parse_args( $this->args['labels'], $defaults ) );
	}
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'hester_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'hester_breadcrumb_trail_items', $this->items, $this->args ) );
	}
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						if ( ! empty( $post_types ) ) {
							...
							// Core filter hook.
							$label = apply_filters( 'hester_post_type_archive_title', $label, $post_type_object->name );
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'hester' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'hester' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'hester' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'hester' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'hester' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'hester' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'hester' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'hester' ) ) );
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' === $tag ) {
					...
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'hester' ) ) ) );
				} elseif ( '%monthnum%' === $tag ) {
					// If using the %monthnum% tag, add a link to the monthly archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'hester' ) ) ) );
				} elseif ( '%day%' === $tag ) {
					// If using the %day% tag, add a link to the daily archive.
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'hester' ) ) ) );
				} elseif ( '%author%' === $tag ) {
