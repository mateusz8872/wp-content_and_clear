...
if ( ! function_exists( 'hester_get_allowed_html_tags' ) ) {
	/**
	 ...
	 */
	function hester_get_allowed_html_tags( $type = 'post' ) {
		...
		return apply_filters( 'hester_allowed_html_tags', $tags, $type );
	}
}
 ...
 */
function hester_option( $id, $prefix = 'hester_', $type = 'theme_mod' ) {

	if ( 'theme_mod' === $type ) {
		return hester()->options->get( $prefix . $id );
	} else {
		return get_option( $prefix . $id, hester()->options->get( $prefix . $id ) );
	}
}
 ...
 * @return mixed Option value.
 */
function hester_get_default( $id, $prefix = 'hester_' ) {
	return hester()->options->get_default( $prefix . $id );
}
 ...
 */
function hester_is_top_bar_displayed( $post_id = 0 ) {

	if ( ! $post_id ) {
		$post_id = hester_get_the_id();
	}

	$top_bar_displayed = hester_option( 'top_bar_enable' );

	if ( $post_id && $top_bar_displayed ) {
		$top_bar_displayed = ! get_post_meta( $post_id, 'hester_disable_topbar', true );
	}
	...
	return apply_filters( 'hester_is_top_bar_displayed', $top_bar_displayed, $post_id );
}
 ...
 */
function hester_is_preloader_displayed() {

	$displayed = (bool) hester_option( 'preloader' );

	return apply_filters( 'hester_is_preloader_displayed', $displayed );
}
 ...
 */
function hester_is_header_displayed( $post_id = 0 ) {
	...
	if ( $post_id ) {
		$displayed = ! get_post_meta( $post_id, 'hester_disable_header', true );
	}

	return apply_filters( 'hester_is_header_displayed', $displayed, $post_id );
}
 ...
 */
function hester_is_header_transparent( $post_id = 0 ) {

	if ( ! $post_id ) {
		$post_id = hester_get_the_id();
	}

	$enabled = hester_option( 'tsp_header' );

	if ( $enabled && hester_is_section_disabled( hester_option( 'tsp_header_disable_on' ), $post_id ) ) {
		$enabled = false;
	}

	if ( $post_id ) {

		$_meta = get_post_meta( $post_id, 'hester_transparent_header', true );
	...
	return apply_filters( 'hester_transparent_header', $enabled, $post_id );
}
 ...
 */
function hester_is_pre_footer_displayed( $post_id = 0 ) {
	...
	if ( ! hester_is_pre_footer_cta_displayed( $post_id ) ) {
		$displayed = false;
	}

	return apply_filters( 'hester_is_pre_footer_displayed', $displayed, $post_id );
}
 ...
 * @return boolean True if Pre Footer is enabled.
 */
function hester_is_pre_footer_cta_displayed( $post_id = 0 ) {
	...
	if ( $displayed && hester_is_section_disabled( hester_option( 'pre_footer_cta_hide_on' ), $post_id ) ) {
		$displayed = false;
	}

	if ( $post_id && $displayed ) {
		$displayed = ! get_post_meta( $post_id, 'hester_disable_prefooter_cta', true );
	}
	...
	return apply_filters( 'hester_is_pre_footer_cta_displayed', $displayed, $post_id );
}
 ...
 */
function hester_is_hero_displayed( $post_id = 0 ) {
	...
	if ( $displayed && ! hester_is_section_disabled( hester_option( 'hero_enable_on' ), $post_id ) ) {
		$displayed = false;
	}

	return apply_filters( 'hester_is_hero_displayed', $displayed, $post_id );
}
 ...
 */
function hester_is_footer_displayed( $post_id = 0 ) {

	if ( ! $post_id ) {
		$post_id = hester_get_the_id();
	}

	$footer_displayed = hester_option( 'enable_footer' );

	if ( $post_id && $footer_displayed ) {
		$footer_displayed = ! get_post_meta( $post_id, 'hester_disable_footer', true );
	}
	...
	if ( $footer_displayed && ! current_user_can( 'edit_theme_options' ) ) {
		...
		$footer_columns = hester_get_footer_column_count();
		$footer_active  = false;

		for ( $i = 1; $i <= $footer_columns; $i++ ) {
			$footer_active = $footer_active || is_active_sidebar( 'hester-footer-' . $i );
		}

		$footer_displayed = $footer_displayed && $footer_active;
	}

	return apply_filters( 'hester_is_footer_displayed', $footer_displayed, $post_id );
}
 ...
 */
function hester_is_copyright_bar_displayed( $post_id = 0 ) {

	if ( ! $post_id ) {
		$post_id = hester_get_the_id();
	}

	$displayed = hester_option( 'enable_copyright' );
	$widgets   = hester_option( 'copyright_widgets' );
	...
	if ( $post_id && $displayed ) {
		$displayed = ! get_post_meta( $post_id, 'hester_disable_copyright', true );
	}
	...
	return apply_filters( 'hester_is_copyright_displayed', $displayed, $post_id );
}
 ...
 */
function hester_is_colophon_displayed( $post_id = 0 ) {
	...
	if ( ! hester_is_footer_displayed( $post_id ) && ! hester_is_copyright_bar_displayed( $post_id ) ) {
		$displayed = false;
	}
	...
	return apply_filters( 'hester_is_colophon_displayed', $displayed, $post_id );
}
 ...
 */
function hester_is_page_header_displayed( $post_id = 0 ) {

	if ( ! $post_id ) {
		$post_id = hester_get_the_id();
	}

	$displayed = hester_option( 'page_header_enable' );

	if ( $post_id && $displayed ) {
		$displayed = hester_page_header_has_title( $post_id ) || hester_page_header_has_breadcrumbs( $post_id );
	} elseif ( is_404() ) {
		$displayed = false;
	}
	...
	return apply_filters( 'hester_is_page_header_displayed', $displayed, $post_id );
}
 ...
 */
function hester_is_woocommerce_activated() {
 ...
 */
function hester_get_sidebar_name_by_id( $sidebar_id = '' ) {
 ...
 */
function hester_hex2rgba( $color, $opacity = false ) {
 ...
 */
function hester_rgba2hex( $color ) {
 ...
 */
function hester_luminance( $hexcolor, $percent ) {
	...
	if ( false !== strpos( $hexcolor, 'rgb' ) ) {
		$hexcolor = hester_rgba2hex( $hexcolor );
	}
...
if ( ! function_exists( 'hester_get_prop' ) ) :
	 ...
	 */
	function hester_get_prop( $array, $prop, $default = null ) {
...
function hester_get_blog_url() {
	...
	return apply_filters( 'hester_site_url', $blog_url );
}
 ...
 */
function hester_design_options_defaults( $options = array() ) {
	...
	return apply_filters( 'hester_design_options_defaults', $defaults, $options );
}
 ...
 * @return array $defaults array of default values.
 */
function hester_typography_defaults( $options = array() ) {

	$defaults = apply_filters(
		'hester_typography_defaults',
		array(
			'font-family'         => 'inherit',
			'font-subsets'        => array(),
			'font-weight'         => '400',
...
if ( ! function_exists( 'hester_enable_page_builder_compatibility' ) ) :
	 ...
	 */
	function hester_enable_page_builder_compatibility() {
		return apply_filters( 'hester_enable_page_builder_compatibility', true );
	}
 ...
 */
function hester_get_background_color() {

	$site_layout = hester_get_site_layout();

	if ( in_array( $site_layout, array( 'boxed', 'boxed-separated' ), true ) ) {
		$background_color = hester_option( 'boxed_content_background_color' );
	} else {
 ...
 */
function hester_is_section_disabled( $disabled_on = array(), $post_id = 0 ) {
 ...
 */
function hester_get_image_sizes() {
...
if ( ! function_exists( 'hester_display_notices' ) ) :
	/**
	 * Display notices.
	 */
	function hester_display_notices() {
		return defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG || defined( 'WP_DEBUG' ) && WP_DEBUG;
	}
endif;

if ( ! function_exists( 'hester_design_common_section' ) ) :
	/**
	 * Common sections that will have design options control in customizer
	 */
	function hester_design_common_section() {
		return apply_filters( 'hester_design_options_common_sections', array( 'features' ) );
	}
endif;

if ( ! function_exists( 'hester_home_sections' ) ) :
	/**
	 * Common sections that will have design options control in customizer
	 */
	function hester_home_sections() {
		return apply_filters( 'hester_home_sections', array( 'info', 'services', 'features', 'blog', 'extra', 'products' ) );
	}
endif;