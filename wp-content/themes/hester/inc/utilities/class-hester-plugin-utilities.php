...
class Hester_Plugin_Utilities {
	...
	/**
	 * Main Hester Plugin Utilities Instance.
	 *
	 * @since 1.0.0
	 * @return Hester_Plugin_Utilities
	 */
	public static function instance() {

		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Hester_Plugin_Utilities ) ) {
	...
	public function activate_plugin( $plugin ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error(
				...
				esc_html__( 'Current user can\'t activate plugins', 'hester' )
			);
		}
		...
		if ( empty( $plugin ) ) {
			return new WP_Error(
				'plugin_activation_failed',
				esc_html__( 'Missing plugin data.', 'hester' )
			);
		}
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					esc_html__( 'Plugin %s is not installed.', 'hester' ),
					$plugin
				)
			);
		}
		...
		do_action( 'hester_plugin_activated_' . $plugin );
	}
	...
	public function deactivate_plugin( $plugin ) {
		...
		if ( empty( $plugin_data ) ) {
			return new WP_Error(
				...
				sprintf(
					// translators: %s is plugin name.
					esc_html__( 'Plugin %s is not active.', 'hester' ),
					$plugin
				)
			);
		}
		...
		do_action( 'hester_plugin_deactivated_' . $plugin );
	}
	...
	public function get_recommended_plugins() {
		...
		return apply_filters( 'hester_recommended_plugins', $plugins );
	}
 ...
 *
 * Example: <?php $hester_plugin_utilities = hester_plugin_utilities(); ?>
 *
 * @since 1.0.0
 * @return object
 */
function hester_plugin_utilities() {
	return Hester_Plugin_Utilities::instance();
}
