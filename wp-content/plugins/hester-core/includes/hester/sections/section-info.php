...
if ( ! function_exists( 'hester_section_info' ) ) {
	function hester_section_info( $enable_section = false ) {
		...
		<section id="hester-info" class="<?php echo esc_attr( $classes ); ?> hester_section_info is-info-init-s0" <?php echo wp_kses_post( $section_style ); ?>>
			<?php
			if ( $classes != '' ) {
									...
									if ( $heading != '' ) {
										?>
											<div id="hester-info-heading" class="h2 title">
												<span class="title-shape"></span><?php echo esc_html( $heading ); ?>
											</div>
											<?php
									}
						...
						<div class="col-xs-12 wow fadeInUp">
							<div class="hester-flex-row gy-4 starter__info-wrapper active__focus-row">
								...
								foreach ( $slides as $key => $slide ) {
									...
									$link     = ( $slide->add_link == true && $slide->linktext != '' ) ? sprintf( '<div class="readmore"><a href="%1$s" tabindex="0"><span>%2$s</span>%3$s</a></div>', esc_url_raw( $slide->link ), esc_html( $slide->linktext ), wp_kses_post( '<i class="fas fa-angle-right"></i>' ) ) : '';
