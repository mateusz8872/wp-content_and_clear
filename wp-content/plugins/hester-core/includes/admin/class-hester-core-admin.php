...
final class Hester_Core_Admin {
	...
	private function includes() {
		...
		// Demo Library.
		require_once HESTER_CORE_PLUGIN_DIR . 'includes/admin/demo-library/class-hester-demo-library.php';
	}
	...
	public function init_admin() {
		...
		add_filter( 'hester_dashboard_navigation_items', array( $this, 'update_navigation_items' ) );

		// Add changelog section.
		add_action( 'hester_after_changelog', array( $this, 'changelog' ) );
	...
	public function add_admin_menu() {
		...
		remove_submenu_page( 'themes.php', 'hester-dashboard' );
		remove_submenu_page( null, 'hester-plugins' );
	...
	public function add_changelog_menu() {
		...
		remove_submenu_page( null, 'hester-changelog' );
	...
	public function update_navigation_items( $items ) {
		...
		$items['dashboard']['url'] = admin_url( 'admin.php?page=hester-dashboard' );
		$items['plugins']['url']   = admin_url( 'admin.php?page=hester-plugins' );
		$items['changelog']['url'] = admin_url( 'admin.php?page=hester-changelog' );
	...
	public function theme_required_notice() {
		...
		echo '<div class="notice notice-warning"><p>' . esc_html__( 'Hester Theme needs to be installed and activated in order to use Hester Core plugin.', 'hester-core' ) . ' <a href="' . esc_url( admin_url( 'themes.php' ) ) . '"><strong>' . esc_html__( 'Install & Activate', 'hester-core' ) . '</strong></a>.</p></div>';
	}
