...
if ( ! class_exists( 'Hester_Core' ) ) {
	...
	final class Hester_Core {
		...
		private function includes() {
			...
			// Global includes.
			require_once HESTER_CORE_PLUGIN_DIR . 'includes/widgets/widgets.php';

			require_once HESTER_CORE_PLUGIN_DIR . 'includes/admin/class-hester-core-admin.php';

			if ( defined( 'WP_CLI' ) && WP_CLI ) {
				require_once HESTER_CORE_PLUGIN_DIR . 'includes/cli/class-hester-core-cli.php';
			}

			// theme specific inlcude
			$theme = wp_get_theme(); // gets the current theme
			if ( 'Hester' == $theme->name ) {
				require_once 'includes/hester/hester.php';
			}
		}
	...
	$theme = wp_get_theme();
	if ( 'Hester' === $theme->name || 'Hester Pro' === $theme->name || 'hester' === $theme->template || 'hester-pro' === $theme->template ) {
		hester_core();
	} else {
	...
	function hester_core_theme_notice() {
		echo '<div class="notice notice-warning"><p>' . __( 'Please activate Hester Theme before activating Hester Core.', 'hester-core' ) . '</p></div>';
	}
}
